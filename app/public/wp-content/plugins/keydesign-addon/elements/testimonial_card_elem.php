...
if (!class_exists('KD_ELEM_TESTIMONIAL_CARD')) {
    ...
    class KD_ELEM_TESTIMONIAL_CARD extends KEYDESIGN_ADDON_CLASS {
        ...
        public function kd_testimonial_card_shrt($atts, $content = null) {
            ...
            $wrapper_class = implode(' ', array('key-tcards', 'single-tcard-elem', $card_id, $tcards_author_details, $css_animation, $tcards_extra_class));
            ...
            $output = '<div class="' . trim($wrapper_class) . '" '.$animation_delay.'>';
                ...
                if (!empty($tcards_testimonial_bg_color) && $tcards_author_details == "author-bottom") {
                  $output .= '<style>.key-tcards.author-bottom.'.$card_id.' .tcards_message:before { border-color: '.$tcards_testimonial_bg_color.' transparent transparent '.$tcards_testimonial_bg_color.'; }</style>';
                }
                if (!empty($tcards_testimonial_bg_color) && $tcards_author_details == "author-top") {
                  $output .= '<style>.key-tcards.author-top.'.$card_id.' .tcards_message:before { border-color: transparent transparent '.$tcards_testimonial_bg_color.' '.$tcards_testimonial_bg_color.'; }</style>';
                }
                $output .= '</div>';
            return $output;
